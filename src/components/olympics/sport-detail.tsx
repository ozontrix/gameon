...
/**
 * Game On Olympics — sport detail.
 *
 * Shows the format, rules and every category (with its fee) for one sport, and
 * hands the chosen category to the booking flow.
 */

import { useState } from "react";
import { useRouter } from "next/navigation";
import { ArrowRight, Check, Clock, MapPin, Timer, Trophy, Users } from "lucide-react";
import { findSport, formatINR, type SportId } from "./data";
import { useOlympicsBooking } from "./booking-context";
import { Button, Chip, IconTile, Kicker, Panel, ScreenHeader, StepFooter } from "./ui";
import { cn } from "@/lib/utils";

/** Soft brand bloom in the corner of the intro panel. */
...
export function SportDetail({ sportId }: { sportId: SportId }) {
  const router = useRouter();
  const { draft, startBooking } = useOlympicsBooking();
  ...
  const handleContinue = () => {
    ...
    router.push("/gameon-olympics/book/slot");
  };
  ...
  return (
    <div>
      <ScreenHeader
        ...
        backHref="/gameon-olympics/sports"
