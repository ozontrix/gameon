...
// ─── Early-access signup modal — shared by the home page and every sub-page ───
// The card only mounts while it is open, so each visit starts from a clean form.
export function NotifyModal({ open, onClose }: NotifyModalProps) {
  return <AnimatePresence>{open && <NotifyCard onClose={onClose} />}</AnimatePresence>;
}

function NotifyCard({ onClose }: { onClose: () => void }) {
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            >
              ...
              >
                We&apos;ve saved your spot. You&apos;ll be the first to know when we open our doors — and
                you&apos;ll get exclusive early-access member pricing.
              </motion.p>
              <motion.div
