...
/**
 * Game On Olympics — step 5: confirmation.
 *
...
function bookingRef(sport: SportId | null, categoryId: string | null, slot: string | null) {
  ...
  return `GO-O${Math.abs(value).toString(36).toUpperCase().slice(0, 6).padEnd(6, "4")}`;
}
...
export default function OlympicsSuccessPage() {
  ...
  const { draft, ready, sport, category, pricing, reset } = useOlympicsBooking();
  ...
  if (ready && (!sport || !category)) {
    return (
      <EmptyState
        ...
        copy="Once you complete an entry, your Game On Olympics pass and QR code will show up on this screen."
  ...
  return (
    <div>
      <ScreenHeader title="You're in!" subtitle="Your entry is confirmed" backHref="/gameon-olympics" />
      ...
      <div className="flex flex-col gap-2.5 sm:flex-row">
        <Button
          onClick={() => {
            reset();
            router.push("/gameon-olympics/bookings");
          }}
          size="lg"
          className="flex-1"
        >
